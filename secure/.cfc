...
function onRequestStart(targetPage){
  ...
  if (refind("^/secure/sso/(acs\.cfm|metadata/)$", p)) return true; // let ACS run
  if (left(p,8) eq "/secure/" && !structKeyExists(session,"authenticated")){
    if (p neq "/secure/login.cfm"){ location url="/secure/login.cfm" addtoken=false; return false; }
  }
  return true;
}
    ...
    <cflogin> 
        ...
        <cfelse> 
            <cfif cflogin.name IS "" OR cflogin.password IS ""> 
                ...
                </cfoutput>              <cfinclude template="login.cfm">  
                <cfabort> 
            <cfelse>
                <!--- SHA1 (Apache) Hash --->
